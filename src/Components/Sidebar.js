...
import { openSendMessageDialog, selectMailListCount, selectIsExpandSideBar } from '../features/mailSlice';
...
function Sidebar() {
    ...
    const expandedSideBar = useSelector(selectIsExpandSideBar);

    return (
        <div className={`sidebar ${expandedSideBar ? "sidebar--expanded" : "sidebar--shrinked"}`}>
            <div className='compose__button_container'>
                <Button onClick={() => dispatch(openSendMessageDialog())} className='compose__button'> <img src={ComposeIcon} alt="Compose Icon" /> <span>Compose</span></Button>
            </div>
